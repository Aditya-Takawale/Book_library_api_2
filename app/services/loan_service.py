...
class LoanService:
    ...
    @staticmethod
    def create_loan(db: Session, loan_data: BookLoanCreate, librarian_id: int) -> BookLoan:
        ...
        existing_loan = db.query(BookLoan).filter(
            and_(
                BookLoan.book_id == loan_data.book_id,
                BookLoan.user_id == loan_data.user_id,
        ...
        logger.info(f"Loan created: Book {loan_data.book_id} to User {loan_data.user_id}")
        return loan
