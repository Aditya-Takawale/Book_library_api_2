...
def create_tokens(user_data: dict) -> dict:
    ...
    logger.debug(f"Created tokens for {user_data['email']}: access_token length={len(access_token)}")
    return tokens_response

def verify_token(token: str, token_type: str = "access"):
    """Verify and decode a JWT token with session validation"""
    try:
        logger.debug(f"Verifying token: {token[:20]}... (type: {token_type})")
        
        if not token or len(token.split('.')) != 3:
            logger.warning(f"Invalid token format: token has {len(token.split('.')) if token else 0} segments")
            raise HTTPException(
        ...
        if session_id and session_id in active_sessions:
            ...
            logger.warning(f"Session {session_id} not found in active sessions. Active sessions: {list(active_sessions.keys())}")
            # For development: auto-recreate session if token is valid
            logger.info(f"Auto-recreating session for development: {session_id}")
            active_sessions[session_id] = {
                "user_email": email,
                "user_role": role,
                "is_active": True,
